        ...
        <div class="flex h-screen bg-gray-100">
            ...
            <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
                ...
                <nav class="flex-1 px-2 py-4 space-y-2 overflow-y-hidden hover:overflow-y-auto">
                    <x-dropdown.simple.option class="w-full">
                        ...
                        <x-slot name="content">
                            <x-dropdown.simple.link :href="route('products.index')">
                                {{ __('List Products') }}

                            </x-dropdown.simple.link>

                            <x-dropdown.simple.link :href="route('welcome')">
                                {{ __('Categories') }}
                    ...
                    <x-dropdown.simple.option class="w-full">
                        <x-slot name="header">
                            <x-icons.guard />
                            <span>{{ __('My Record') }}</span>
                        </x-slot>
                        <x-slot name="content">
                            <x-dropdown.simple.link :href="route('products.shopping')">
                                {{ __('My car shopping') }}

                            </x-dropdown.simple.link>
                            <x-dropdown.simple.link :href="route('products.history')">
                                {{ __('Mi historial de compras') }}
                            </x-dropdown.simple.link>
                        </x-slot>
                    </x-dropdown.simple.option>
                    @can('viewAny', App\Models\Product::class)
                        
                        <x-dropdown.simple.option class="w-full" :isActive="request()->routeIs('product.*')">
                            ...
                            <x-slot name="content">
                                <x-dropdown.simple.link :href="route('custom.index')" >{{ __('List Sales') }}</x-dropdown.simple.link>
                                <x-dropdown.simple.link :href="route('custom.create')">{{ __('Create a new post') }}</x-dropdown.simple.link>
                            </x-slot>
                        </x-dropdown.simple.option>
                    ...
                    <x-dropdown.simple.option class="w-full">
                        ...
                        <x-slot name="content">
                            <x-dropdown.simple.link :href="route('admin.categories.index')">
                                {{ __('List Categories') }}

                            </x-dropdown.simple.link>

                            <x-dropdown.simple.link :href="route('admin.tags.index')">
                                {{ __('List Tags') }}
            ...
            <div class="flex items-center space-x-4">
                ...
                <x-dropdown.menu.option>
                    ...
                    <x-slot name="content">
                        <x-dropdown.menu.link :href="route('profile.notice')">
                            {{ __('Notifications') }}
